function [ mintheta, thetas, errors, minrho, minidx, minval ] = graph_estimation( data, lambda, limit, max_iters )
...
parfor m=1:min(n*(n-1),limit)
    ...
    rhos(m,:) = rho;
    %theta(theta<val) = 0; %comment block for hard thresholding
    %theta(theta>=val) = 1; %comment block for hard thresholding
