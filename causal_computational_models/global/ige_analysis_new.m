function [ out ] = ige_analysis_new( grn, exp, metric, figname )
...
size(metric)
[adj_found, all_adjs, all_errors] = graph_estimation_new(metric); %juggler(metric); %can also append '_multiattribute'
adj_found
...
if nargin>=4
    ...
    title('ROC plot');
    print(strcat(figname,'ROCplotIGEnew'),'-dpng');
    figure(2); clf;
    hold;
    ...
    print(strcat(figname,'ScatterIGEnew'),'-dpng');
end
end
